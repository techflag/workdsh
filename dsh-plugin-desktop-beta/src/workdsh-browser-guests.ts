/** WorkDSH carrier for the official Sidebar Browser webview contract.
 * Adapted from the MIT-licensed DeepSeek Harness Desktop browser guest policy.
 */
import { randomUUID } from 'node:crypto'
import { app, session, type BrowserWindow, type Session, type WebContents } from 'electron'

type DesktopBrowserLeaseId = string
interface DesktopBrowserReservation { readonly lease: DesktopBrowserLeaseId; readonly partition: string }
interface DesktopBrowserOpenRequest { readonly lease: DesktopBrowserLeaseId; readonly url: string }
export const WORKDSH_BROWSER_IPC = {
...
export class WorkDSHBrowserGuests {
    ...
    owner.on('did-attach-webview', (_event, guest) => {
      ...
      guest.setWindowOpenHandler(({ url, postBody }) => {
        ...
        if (attachedLease !== undefined && lease?.guest === guest && lease.owner === owner && !owner.isDestroyed()
          ...
          owner.send(WORKDSH_BROWSER_IPC.openRequested, request)
        }
        return { action: 'deny' }
      })

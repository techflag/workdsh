...
`master` 原先承载 Desktop，`main` 承载 Web 与功能包；它们是两套独立的 Git 历史，直接删除任一分支都会丢失源码。迁移以 Desktop `d3a7b2143d27` 为第一父提交，并通过 `git subtree add --prefix=workdsh-web github/main` 将 Web `03b63ea14374` 作为第二父提交导入。合库 PR 以现有默认及发布分支 `main` 为目标；合入后 `main` 同时保留两边的历史和源码。
...
2. 合入 `main` 后核对 Desktop 发布工作流和网站部署都从 `main` 运行；确认不再使用旧 Desktop 分支后删除远程 `master`。GitHub 默认分支保持 `main`。
3. 浏览器侧栏实验存在发布阻断，实验判断已保存在 [审计记录](WORKDSH-BROWSER-SIDEBAR-AUDIT.md)。实验代码不进入发行线；确认不再继续该分支后，删除远程实验分支。

完成迁移后只保留 GitHub 默认分支作为长期产品主线。今后所有过程分支经审查和验证后合回该分支；发布 tag 只指向该分支可达的提交，已合并的过程分支及时删除。此规则也写入根目录 `AGENTS.md`。

...
| 官方能力 | Slot：`https://github.com/deepseek-ai/deepseek-harness/blob/dsh-v0.1.6-alpha.2/docs/subsystems/slots.md:25-41`（`conversation.session.header.actions` 注册示例）、:150（槽清单），原生所有者 ui-conversation。事件：`https://github.com/deepseek-ai/deepseek-harness/blob/dsh-v0.1.6-alpha.2/docs/persistence-catalog.md:473-477`（`deliverables/presented`，log-only）、`https://github.com/deepseek-ai/deepseek-harness/blob/dsh-v0.1.6-alpha.2/docs/tool-catalog.md:624-630`（present 语义：写入后的文件必须 present 才是用户交付）、:25（`@deepseek-ai/dsh-tool-present` 行）。发布包与公开入口：`@deepseek-ai/dsh-client-ui-session/client`（SessionStandardProps.sessionId）、`@deepseek-ai/dsh-tool-present/types`（`PresentedFile`）、`@deepseek-ai/dsh-session`（`session/event`、`SessionHeader.cwd`）、`@deepseek-ai/dsh-fs`（`ctx.fs.resolve/readBytes`）、`@deepseek-ai/dsh-client-ui-layout`（`selectPanel`）。 |

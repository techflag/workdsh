...
涉及 D04 专家或 D11 专家团时，先读 [专家开发入口](docs/design/experts/README.md)，再核对当前源码、官方锁定版本的公开接口与实际验收记录。旧 HLD、0.1.5/0.1.6 探针和历史交接文本不作为现行 API 依据。单专家 D04 与专家团 D11 分开验收，企业服务器/管理 Web 后置。一个专家插件管理多个专家对象；不新增每专家 npm 包或第二套执行器。详情弹框复用公共 UI，原生任务框保留 /、@、附件、模型与权限。

产品语义以 [专家需求](docs/design/experts/PRD.md) 为准：Skill 是能力，专家承担领域判断和交付责任；专家团协作由官方 Agent Teams 执行，WorkDSH 仅管理专家资产与业务指导。使用详情与编辑分开；专家选择真实技能对象。验收包含实际业务成果与必要追问，不只检查 prompt/按钮/工具成功。当前开发边界见 [有限开发计划](docs/design/experts/DEVELOPMENT-PLAN.md)；不另造 Agent loop、团队运行状态机或通用流程引擎。
...
任何页面、图标或样式变更前必须阅读 docs/UI-DESIGN.md。所有插件共用该规范，不自行另起视觉风格。以实际运行的 Host 页面及原生组件为视觉验收对象；结束时按规范第 8 节记录视觉对照、交互验证与未完成项。
